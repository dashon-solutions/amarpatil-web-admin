                        ...
                        <div key={inv._id} className="p-3 border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-900 shadow-sm relative overflow-hidden group">
                          ...
                          <div className="flex gap-2">
                            {inv.pdfUrl && (() => {
                              ...
                              const fullPdfUrl = `${backendBaseUrl}${inv.pdfUrl}?tenant=${window.location.hostname}`;
                              return (
                              <>
                                <a href={fullPdfUrl} target="_blank" rel="noreferrer" className="flex-1 flex justify-center items-center py-2 bg-slate-100 dark:bg-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 text-slate-700 dark:text-slate-300 hover:text-indigo-600 text-[10px] font-bold uppercase tracking-wider rounded transition-colors group-hover:border-indigo-500/30">
                                  <Download className="w-3 h-3 mr-1" /> PDF
                                </a>
                                <button
                                  onClick={() => {
                                    const msg = `Hello ${event.clientName},%0A%0AYour payment invoice (${inv.type}) is ready.%0AAmount: Rs. ${inv.amount}%0A%0ADownload here:%0A${fullPdfUrl}%0A%0A- Our Studio`;
                                    window.open(`https://wa.me/?text=${msg}`, '_blank');
                                  }}
                                  className="flex-1 flex justify-center items-center py-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-400 dark:hover:bg-emerald-500/20 text-[10px] font-bold uppercase tracking-wider rounded transition-colors">
                                  <Smartphone className="w-3 h-3 mr-1" /> Send
                                </button>
                              </>
                              );
                            })()}
